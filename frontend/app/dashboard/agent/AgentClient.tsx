...
export function AgentClient() {
    ...
    const [dryRun, setDryRun] = useState(true); // Güvenlik: varsayılan olarak simülasyon modu açık
            ...
            <div className="relative p-10 rounded-[32px] bg-bg-elevated border border-border-subtle overflow-hidden">
                ...
                <div className="relative z-10 space-y-6">
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                        ...
                        {/* Dry-run / Canlı modu toggle */}
                        <button
                            onClick={() => setDryRun(v => !v)}
                            className={`h-10 px-5 rounded-full text-xs font-bold border transition-all flex items-center gap-2 ${
                                dryRun
                    ...
                    {!dryRun && (
                        <p className="text-xs text-red-400 font-medium">
                            ⚠ Canlı mod aktif — &quot;Uygula&quot; butonuna basıldığında değişiklikler doğrudan Etsy&apos;ye yazılır.
                        </p>
                    )}
                </div>
            </div>
                                ...
                                <div key={i} className="group p-5 rounded-2xl bg-bg-elevated border border-border-subtle hover:border-accent/40 transition-all flex flex-col justify-between">
                                    ...
                                    <div className="mt-6 flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className={`flex-1 h-9 rounded-lg text-xs font-bold hover:scale-[1.02] transition-all disabled:opacity-50 ${
                                                dryRun
                                                    ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40'
                                                    : 'bg-accent text-white'
                                            }`}
                                        >
                                            {executingId === action.listingId
                                                ? <Loader2 className="w-3 h-3 animate-spin mx-auto" />
                                                : dryRun ? 'Simüle Et' : 'Uygula (Canlı)'}
                                        </button>
                                    </div>
                                </div>
                            ))}
