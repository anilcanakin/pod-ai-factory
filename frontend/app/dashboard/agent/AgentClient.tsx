'use client';
  ...
  import { Bot, Zap, Search, CheckCircle2, AlertTriangle, TrendingUp, DollarSign, Tag, ImageIcon, Loader2, Play, ShieldCheck } from 'lucide-react';
  ...
  export function AgentClient() {
      ...
      const handleApplyAction = async (action: any) => {
          ...
          const modeLabel = dryRun ? 'Simülasyon' : 'Canlı';
          const toastId = toast.loading(`[${modeLabel}] ${action.actionType.replace('_', ' ')} uygulanıyor...`);
          try {
              const result = await apiAgent.applyAction(action, dryRun);
              if (result.dryRun) {
                  toast.success(`Simülasyon tamamlandı: ${result.preview}`, { id: toastId });
              } else {
                  toast.success("Etsy'de başarıyla uygulandı!", { id: toastId });
                  if (plan) {
                      setPlan({ ...plan, actions: plan.actions.filter(a => a !== action) });
                  }
              }
          } catch (err) {
              toast.error('Uygulama başarısız: ' + (err as Error).message, { id: toastId });
          } finally {
              setExecutingId(null);
          }
      };
              ...
              <div className="relative p-10 rounded-[32px] bg-bg-elevated border border-border-subtle overflow-hidden">
                  ...
                  <div className="relative z-10 space-y-6">
                      ...
                      <p className="max-w-xl text-text-secondary leading-relaxed">
                          The AI Manager uses your Multimodal Corporate Brain and combines it with Live Etsy Performance Statistics to identify high-impact optimizations.
                      </p>
                      <div className="flex items-center gap-4 flex-wrap">
                          <button onClick={runAudit} disabled={running} className="h-12 px-8 rounded-full bg-accent text-white font-bold flex items-center gap-3 hover:scale-[1.02]
  active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100">
                              {running ? <Loader2 className="w-5 h-5 animate-spin" /> : <Play className="w-5 h-5" />}
                              {running ? 'Denetim Sürüyor...' : 'Mağaza Denetimi Başlat'}
                          </button>
                          <button onClick={() => setDryRun(v => !v)} className={`h-10 px-5 rounded-full text-xs font-bold border transition-all flex items-center gap-2 ${dryRun ? 'bg-amber-500/10  border-amber-500/40 text-amber-400 hover:bg-amber-500/20' : 'bg-red-500/10 border-red-500/40 text-red-400 hover:bg-red-500/20'}`}>
                              <ShieldCheck className="w-3.5 h-3.5" />
                              {dryRun ? 'Simülasyon Modu (Güvenli)' : "Canlı Mod (Etsy'ye Yazar!)"}
                          </button>
                      </div>
                      {!dryRun && (
                          <p className="text-xs text-red-400 font-medium">Canlı mod aktif — Uygula butonuna basıldığında değişiklikler doğrudan Etsy'ye yazılır.</p>
                      )}
                  </div>
              </div>
                                      ...
                                      <div className="space-y-4">
                                          <div className="flex items-start justify-between">
                                              <div className="flex items-center gap-2">
                                                  <div className={`p-2 rounded-lg ${action.actionType === 'UPDATE_PRICE' ? 'bg-green-500/10 text-green-500' : action.actionType === 'UPDATE_SEO' ?  
  'bg-blue-500/10 text-blue-500' : action.actionType === 'UPDATE_MOCKUP' ? 'bg-purple-500/10 text-purple-500' : 'bg-amber-500/10 text-amber-500'}`}>
                                                  ...
                                                  <span className="text-[10px] font-black uppercase tracking-widest opacity-60">{action.actionType.replace('_', ' ')}</span>
                                              </div>
                                              <span className="text-[10px] bg-bg-base px-2 py-0.5 rounded text-text-tertiary font-mono">ID: {action.listingId}</span>
                                          </div>
                                          <p className="text-sm font-medium text-text-primary group-hover:text-accent transition-colors">{action.reason}</p>
                                      ...
                                      <div className="mt-6 flex gap-2">
                                          <button className="flex-1 h-9 rounded-lg bg-bg-overlay border border-border-subtle text-xs font-bold text-text-secondary hover:text-text-primary
  hover:bg-bg-base transition-all">Yoksay</button>
                                          <button onClick={() => handleApplyAction(action)} disabled={executingId === action.listingId} className={`flex-1 h-9 rounded-lg text-xs font-bold
  hover:scale-[1.02] transition-all disabled:opacity-50 ${dryRun ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-accent text-white'}`}>
                                              {executingId === action.listingId ? <Loader2 className="w-3 h-3 animate-spin mx-auto" /> : dryRun ? 'Simüle Et' : 'Uygula (Canlı)'}
                                          </button>
                                      </div>
                                  </div>
                              ))}
