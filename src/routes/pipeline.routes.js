        ...
        if (options.seo !== false) {
            try {
                ...
                const seoResponse = await client.messages.create({
                    ...
                    max_tokens: 4096,
                    system: `${knowledge}\n\nCRITICAL: Return ONLY a single raw JSON object. No markdown, no code fences, no explanations. Output must start with { and end with }.\n\nRULES:\n- title: MUST be 70-90 characters (count every character including spaces). If under 70, add descriptive words. First 50 chars must contain primary long-tail keyword. Use pipe | separator for secondary keyword.\n- description: max 800 characters, problem→solution format, NO bullet points or lists\n- tags: exactly 13 items, each tag MAX 20 characters INCLUDING spaces (count every character). WRONG: "patriotic t shirt men" (21 chars). RIGHT: "patriotic tshirt" (16 chars), "eagle shirt" (11 chars), "freedom tee" (11 chars). Short 2-3 word phrases only, all lowercase (no capital letters)\n\nFormat: {"title":"...","description":"...","tags":["tag1","tag2","tag3","tag4","tag5","tag6","tag7","tag8","tag9","tag10","tag11","tag12","tag13"],"topKeywords":["kw1","kw2","kw3"]}`,
                    messages: [{
                        role: 'user',
